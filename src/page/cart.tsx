...
export default function Cart() {
    ...
    const handleDelete = (ProductId: number) => {
        ...
        localStorage.setItem('items', JSON.stringify(updatedItems)); // Cập nhật localStorage nếu cần
    };
    ...
    useEffect(() => {
        ...
        for (let i = 0 ; i < items.length ; i++) {
            ...
            setSubTotal(items[i].Price)
            setTotal(items[i].Price + 5)
        }
    }, []);
    ...
    if(!items) {
        return(
            <>
                <p className="text-center mt-8">adasdasd</p>
            </>
        )
    }
